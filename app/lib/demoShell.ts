...
export const SNIPPETS = [
  {
    ...
    desc: "Reattach the long-running session",
    tag: "TMUX",
  },
  {
    cmd: "journalctl -u caddy -n 80 --no-pager",
    desc: "Last 80 lines of the web server unit",
    tag: "LOGS",
  },
  {
    cmd: "docker compose up -d --build",
    desc: "Rebuild and restart the stack",
    tag: "DEPLOY",
  },
  { cmd: "pkg upgrade -y", desc: "Update every Termux package", tag: "PKG" },
  {
    cmd: "df -h | sort -k5 -r | head",
    desc: "Fullest mounts first",
    tag: "DISK",
  },
  {
    cmd: "systemctl restart caddy",
    desc: "Bounce the reverse proxy",
    tag: "OPS",
  },
];
...
const OUT: Record<string, string> = {
  ...
  "tmux ls":
    "main: 3 windows (created Mon Jul 21 08:14:02 2026) (attached)\nbuild: 1 windows (created Fri Aug 28 22:03:19 2026)",
  "git status":
...
export function runDemoCommand(cmd: string, prompt: string): Line[] {
  ...
  if (OUT[cmd]) add.push({ k: "out", text: OUT[cmd] });
    ...
    add.push({ k: "sys", text: "now in " + cmd.slice(3) });
  else if (cmd.indexOf("tmux attach") === 0)
    add.push({ k: "sys", text: "[attached to session main — 3 windows]" });
  else
    add.push({
      k: "out",
      text: "zsh: command not found: " + cmd.split(" ")[0],
    });
  return add;
}
...
export const ROWS = [
  {
    ...
    keys: [
      { label: "ESC", sub: "", a: "esc" },
      { label: "⇥", sub: "TAB", a: "tab" },
      { label: "CTRL", sub: "HOLD", a: "ctrl" },
      { label: "ALT", sub: "HOLD", a: "alt" },
      { label: "^C", sub: "KILL", a: "sigint" },
      { label: "^D", sub: "EOF", a: "eof" },
      { label: "^L", sub: "CLR", a: "clear" },
      { label: "^Z", sub: "BG", a: "bg" },
    ],
  },
  {
    ...
    keys: [
      { label: "←", sub: "", a: "left" },
      { label: "↓", sub: "", a: "histdown" },
      { label: "↑", sub: "HIST", a: "histup" },
      { label: "→", sub: "", a: "accept" },
      { label: "⇱", sub: "HOME", a: "home" },
      { label: "⇲", sub: "END", a: "end" },
      { label: "⇞", sub: "PGUP", a: "pgup" },
      { label: "⇟", sub: "PGDN", a: "pgdn" },
    ],
  },
  {
    label: "TMUX",
    keys: [
      { label: "⌃B", sub: "PFX", a: "noop" },
      { label: "⊟", sub: "SPLIT", a: "split" },
      { label: "⊞", sub: "VSPL", a: "split" },
      { label: "⇄", sub: "PANE", a: "noop" },
      { label: "＋", sub: "WIN", a: "newtab" },
      { label: "⌕", sub: "COPY", a: "copy" },
      { label: "⤢", sub: "ZOOM", a: "zoom" },
      { label: "⏻", sub: "DTCH", a: "detach" },
    ],
  },
] as const;
