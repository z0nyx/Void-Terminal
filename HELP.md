...
(sshj для Android, NMSSH для iOS), потому что единственный существующий
npm-пакет для SSH в React Native (`react-native-ssh-sftp`) заброшен и
несовместим с текущей версией React Native. Код модуля написан
внимательно, но **ни разу не компилировался** — в среде, где создавался
проект, не было Android SDK и Xcode. Это значит:
...
- Реальный протокол mosh, полноценная интеграция управляющих
  последовательностей tmux и точная побайтовая передача на iOS —
  задокументированные ограничения текущей версии, см. `README.md`.
- Для сборки под iOS (`.ipa`) нужен Mac с Xcode и учётная запись Apple
  Developer Program — это отдельный процесс, не описанный в этом
  документе (он посвящён именно Android/`.apk`).
